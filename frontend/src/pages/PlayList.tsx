...
const PlayList = () => {
  ...
  useEffect(() => {
    if (songs && songs.length > 0 && user?.playlist) {
      const filteredSongs = songs.filter((song) => {
        return user.playlist.some(
          (id) => String(id) === String(song._id || song.id),
        );
      ...
      <Layout>
        {loading ? (
          ...
          <>
            ...
            {/* Logic for Empty Playlist */}
            {myPlayList.length > 0 ? (
              myPlayList.map((song, index) => (
                <div
                  className="grid grid-cols-3 sm:grid-cols-4 mt-4 mb-4 pl-2 py-2 text-[#a7a7a7] hover:bg-[#ffffff2b] rounded cursor-pointer transition"
              ...
              <div className="mt-20 text-center">
                ...
                <p className="text-[#a7a7a7] mt-2">
                  Start adding some of your favorites!
                </p>
              </div>
            )}
          </>
